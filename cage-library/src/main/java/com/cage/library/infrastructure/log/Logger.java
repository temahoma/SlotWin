package com.cage.library.infrastructure.log;
...
public final class Logger {
    ...
    private static void logHeaderContent(int logType, String tag, int methodCount) {
        ...
        for (int i = methodCount; i > 0; i--) {
            ...
            String name = getSimpleClassName(trace[stackIndex].getClassName());

            if (name.startsWith("ZygoteInit")
                    || name.equals("Logger")
                    || name.equals("Looper")
                    || name.equals("Log")
                    || name.equals("Handler")
                    || name.equals("WeakRefRunnable")
                    || name.equals("Executors$RunnableAdapter")
                    || name.equals("Dispatcher")) {
                continue;
            }
