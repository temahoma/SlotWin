...
public class ResourceHelper {
    ...
    public static Resources.Theme getTheme(){
        return CageLibrary.getAppContext().getTheme();
    }
    ...
    public static ColorStateList getColorStateList(@ColorRes int resId) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            return resources.getColorStateList(resId, getTheme());
        } else {
    ...
    public static Drawable getDrawable(@DrawableRes int resId) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            return resources.getDrawable(resId, getTheme());
        } else {
    ...
    private static int getIdentifier(String name, @IdentifierType String type) {
    ...
    public static int getIdentifier(String name, @IdentifierType String type, int fallback) {
        ...
        int id = resources.getIdentifier(name, type, CageLibrary.getAppContext().getPackageName());
        if (id <= 0) {
            return fallback;
        }
        return id;
